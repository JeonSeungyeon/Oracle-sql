...
select trunc(sysdate - to_date (20220101, 'YYYYMMDD')) as 올해날짜수
from dual;

-- 5-1. 자신이 태어난 날짜에서 현재까지 몇 일이 지났는지 출력 하세요. 
select trunc(sysdate - to_date (20020222, 'YYYYMMDD')) as 산날
from dual;

-- 5-2. 자신이 태어난 날짜에서 현재까지 몇 개월이 지났는지 출력 하세요. 
select trunc (months_between (sysdate, to_date (20020222, 'YYYYMMDD'))) as 산달
from dual;
...
select eno, rpad(substr(eno, 1, 2), 4, '*') as 가린번호,
ename, rpad(substr(ename, 1, 1), length(ename),  '*') as 가린이름
from employee; 
